...
async function checkRedis() {
  ...
  try {
    ...
    redisAvailable = true
    console.log('[Realtime] Redis connected — using distributed Pub/Sub')
  } catch {
    redisAvailable = false
    console.log('[Realtime] Redis unavailable — using in-memory EventEmitter (single-instance mode)')
  }
  ...
  const stream = new ReadableStream({
    async start(controller) {
      try {
        ...
        console.error('[SSE] Stream init error:', err.message)
        controller.enqueue(encoder.encode(
