...
/**
 * Resolve avatar to direct CDN URL (not /api/media/ proxy).
 * Looks up media_assets for publicUrl. Falls back to profilePicUrl or /api/media/.
 * This is async — use in contexts where DB is available.
 */
export async function resolveAvatarUrl(db, user) {
  if (!user) return ''
  // Priority: profilePicUrl (already CDN) > media_assets publicUrl > /api/media/ proxy
  if (user.profilePicUrl) return user.profilePicUrl
  const mid = user.avatarMediaId || user.avatar || null
  if (!mid) return ''
  // Lookup CDN URL from media_assets
  if (db) {
    try {
      const asset = await db.collection('media_assets').findOne(
        { id: mid, isDeleted: { $ne: true } },
        { projection: { _id: 0, publicUrl: 1 } }
      )
      if (asset?.publicUrl) return asset.publicUrl
    } catch {}
  }
  return `/api/media/${mid}`
}
...
export function toUserSnippet(user) {
  if (!user) return null
  const mid = user.avatarMediaId || user.avatar || null
  // World best: profilePicUrl is ALWAYS a direct CDN URL (set at upload time)
  // Only fall back to /api/media/ proxy if no profilePicUrl exists
  const avatarUrl = user.profilePicUrl || resolveMediaUrl(mid) || ''
  return {
    id: user.id,
    displayName: user.displayName || null,
    username: user.username || null,
    avatarUrl,
    profilePicUrl: user.profilePicUrl || avatarUrl,
    avatarMediaId: mid,
    avatar: mid,
    role: user.role || 'USER',
...
export function toUserProfile(user) {
  ...
  return {
    ...safe,
    avatarUrl: safe.profilePicUrl || resolveMediaUrl(mid),
    avatarMediaId: mid,
    avatar: mid,
  }
}
...
/**
 * Async version — resolves avatar to CDN URL from media_assets.
 * Use when DB is available (auth/me, auth/login).
 */
export async function toUserProfileWithCdn(user, db) {
  if (!user) return null
  const profile = toUserProfile(user)
  if (profile.profilePicUrl) {
    profile.avatarUrl = profile.profilePicUrl
    return profile
  }
  const mid = user.avatarMediaId || user.avatar
  if (mid && db) {
    try {
      const asset = await db.collection('media_assets').findOne(
        { id: mid, isDeleted: { $ne: true } },
      ...
      if (asset?.publicUrl) {
        profile.avatarUrl = asset.publicUrl
        profile.profilePicUrl = asset.publicUrl
      }
    } catch {}
  }
  return profile
...
export function toMediaObject(asset) {
  ...
  return {
    id: asset.id,
    url: asset.publicUrl || asset.url || resolveMediaUrl(asset.id),
    publicUrl: asset.publicUrl || null,
    type: asset.type || asset.mimeType || null,
    thumbnailUrl: asset.thumbnailUrl || null,
    posterFrameUrl: asset.posterFrameUrl || asset.thumbnailUrl || null,
    width: asset.width || null,
    height: asset.height || null,
    duration: asset.duration || null,
    mimeType: asset.mimeType || null,
    size: asset.size || asset.sizeBytes || null,
    storageType: asset.storageType || null,
    // Video playback hints
    ...(isVideo ? {
      playbackHints: {
        preload: 'metadata',
        loop: false,
