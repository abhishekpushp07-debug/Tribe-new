...
export async function buildStoryRail(db, user, sanitizeUserFn) {
  ...
  // Enrich with author info + resolve avatar CDN URLs (not /api/media/ proxy)
  const authorIds = Object.keys(grouped)
  const authors = await db.collection('users').find({ id: { $in: authorIds } }).toArray()
  
  // Batch-resolve avatar media assets for CDN URLs
  ...
  const authorMap = Object.fromEntries(authors.map(a => {
    const snippet = sanitizeUserFn(a)
    if (snippet) {
  ...
  const storyRail = authorIds.map(authorId => ({
    ...
    stories: grouped[authorId].stories.map(s => {
      const { _id, ...clean } = s
      let ta = ''
      if (clean.createdAt) {
      ...
      return { ...clean, viewCount: clean.viewCount || 0, reactionCount: clean.reactionCount || 0, timeAgo: ta, formatted: { views: String(clean.viewCount || 0), reactions: String(clean.reactionCount || 0) } }
    }),
