...
// npm-bundled ffmpeg — works on EVERY server without apt-get
// Resolve binary paths dynamically at runtime (avoids webpack processing)
let FFMPEG = 'ffmpeg'
let FFPROBE = 'ffprobe'
try {
  ...
  if (existsSync(ffmpegPath)) FFMPEG = ffmpegPath
  // ffprobe — find the right arch binary
  const ffprobeBase = join(process.cwd(), 'node_modules', '@ffprobe-installer')
  if (existsSync(ffprobeBase)) {
    const { readdirSync } = await import('fs')
    for (const dir of readdirSync(ffprobeBase)) {
      const fp = join(ffprobeBase, dir, 'ffprobe')
      if (existsSync(fp)) { FFPROBE = fp; break }
    }
  }
} catch {}
...
// ═══════════════════════════════════════════════════
// FFMPEG DETECTION — uses npm-bundled binaries (always available)
// ═══════════════════════════════════════════════════
