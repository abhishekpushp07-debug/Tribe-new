...
const ALLOWED_MIME_TYPES = [
  ...
  'audio/mp4', 'audio/mpeg', 'audio/aac', 'audio/wav', 'audio/ogg',
]
...
// Generate TUS resumable upload URL for large files (chunked, resumable)
export async function createResumableUploadUrl(filePath) {
  await ensureBucket()
  const url = process.env.SUPABASE_URL
  const key = process.env.SUPABASE_SERVICE_ROLE_KEY
  
  // Supabase TUS endpoint: POST /storage/v1/upload/resumable
  const tusEndpoint = `${url}/storage/v1/upload/resumable`
