...
import { handleMediaOrchestration } from '@/lib/handlers/media-orchestration'
...
export async function dispatchRoute(path, method, request, db) {
  ...
  switch (segment) {
    ...
    case 'media':
      ...
      return (await handleMediaOrchestration(path, method, request, db))
        || (await handleMedia(path, method, request, db))
