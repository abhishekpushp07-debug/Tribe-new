...
export async function handleEvents(path, method, request, db) {
  ...
  if (path[0] === 'events' && path.length === 1 && method === 'POST') {
    ...
    if (recentCount >= EVENT_CREATE_RATE_LIMIT) {
      return { error: `Rate limit: max ${EVENT_CREATE_RATE_LIMIT} events per hour`, code: 'RATE_LIMITED', status: 429 }
    }
    ...
    if (!title || !startAt) {
      return { error: 'title and startAt are required', code: 'VALIDATION_ERROR', status: 400 }
    }
    if (title.length > EVENT_TITLE_MAX) {
      return { error: `Title max ${EVENT_TITLE_MAX} chars`, code: 'VALIDATION_ERROR', status: 400 }
    }
    if (description && description.length > EVENT_DESC_MAX) {
      return { error: `Description max ${EVENT_DESC_MAX} chars`, code: 'VALIDATION_ERROR', status: 400 }
    }
    if (!ALLOWED_CATEGORIES.includes(category)) {
      return { error: `Category must be: ${ALLOWED_CATEGORIES.join(', ')}`, code: 'VALIDATION_ERROR', status: 400 }
    }
    ...
    if (isNaN(startDate.getTime())) {
      return { error: 'Invalid startAt date', code: 'VALIDATION_ERROR', status: 400 }
    }
    if (endAt) {
      const endDate = new Date(endAt)
      if (isNaN(endDate.getTime()) || endDate <= startDate) {
        return { error: 'endAt must be after startAt', code: 'VALIDATION_ERROR', status: 400 }
      }
    }
    if (capacity !== undefined && capacity !== null && (capacity < 1 || capacity > 100000)) {
      return { error: 'Capacity must be 1-100000', code: 'VALIDATION_ERROR', status: 400 }
    }
  ...
  if (path[0] === 'events' && path.length === 2 && method === 'GET' && !['feed', 'search', 'college'].includes(path[1])) {
    ...
    if (!event) return { error: 'Event not found', code: 'NOT_FOUND', status: 404 }

    if (event.status === 'REMOVED') {
      return { error: 'This event has been removed', code: 'GONE', status: 410 }
    }
    ...
    if (['HELD'].includes(event.status) && !isOwner && !isAdminUser) {
      return { error: 'Event not available', code: 'FORBIDDEN', status: 403 }
    }
    if (event.status === 'DRAFT' && !isOwner) {
      return { error: 'Event not found', code: 'NOT_FOUND', status: 404 }
    }
  ...
  if (path[0] === 'events' && path.length === 2 && method === 'PATCH' && !['feed', 'search', 'college'].includes(path[1])) {
    ...
    if (['REMOVED', 'CANCELLED'].includes(event.status)) {
      return { error: `Cannot edit event with status ${event.status}`, code: 'INVALID_STATE', status: 400 }
    }
    ...
    if (body.title !== undefined) {
      if (!body.title || body.title.length > EVENT_TITLE_MAX) return { error: `Title required, max ${EVENT_TITLE_MAX}`, code: 'VALIDATION_ERROR', status: 400 }
    ...
    if (body.visibility !== undefined) {
      if (!ALLOWED_VISIBILITIES.includes(body.visibility)) return { error: 'Invalid visibility', code: 'VALIDATION_ERROR', status: 400 }
  ...
  if (path[0] === 'events' && path.length === 3 && path[2] === 'publish' && method === 'POST') {
    ...
    const event = await db.collection('events').findOne({ id: eventId })
    if (!event) return { error: 'Event not found', code: 'NOT_FOUND', status: 404 }
    if (event.creatorId !== user.id) return { error: 'Not authorized', code: 'FORBIDDEN', status: 403 }
    if (event.status !== 'DRAFT') return { error: `Cannot publish event with status ${event.status}`, code: 'INVALID_STATE', status: 400 }
  ...
  if (path[0] === 'events' && path.length === 3 && path[2] === 'cancel' && method === 'POST') {
    ...
    const event = await db.collection('events').findOne({ id: eventId })
    if (!event) return { error: 'Event not found', code: 'NOT_FOUND', status: 404 }
    if (event.creatorId !== user.id && !isAdmin(user)) return { error: 'Not authorized', code: 'FORBIDDEN', status: 403 }
    if (!['PUBLISHED', 'DRAFT'].includes(event.status)) {
      return { error: `Cannot cancel event with status ${event.status}`, code: 'INVALID_STATE', status: 400 }
    }
  ...
  if (path[0] === 'events' && path.length === 3 && path[2] === 'archive' && method === 'POST') {
    ...
    if (!['PUBLISHED', 'CANCELLED'].includes(event.status)) {
      return { error: `Cannot archive event with status ${event.status}`, code: 'INVALID_STATE', status: 400 }
    }
  ...
  if (path[0] === 'events' && path.length === 3 && path[2] === 'rsvp' && method === 'POST') {
    ...
    if (!event) return { error: 'Event not found or not published', code: 'NOT_FOUND', status: 404 }

    // Block check
    if (await isBlocked(db, user.id, event.creatorId)) {
      return { error: 'Action not allowed', code: 'FORBIDDEN', status: 403 }
    }

    const body = await request.json()
    const { status } = body
    if (!status || !ALLOWED_RSVP.includes(status)) {
      return { error: `status must be: ${ALLOWED_RSVP.join(', ')}`, code: 'VALIDATION_ERROR', status: 400 }
    }
  ...
  if (path[0] === 'events' && path.length === 3 && path[2] === 'rsvp' && method === 'DELETE') {
    ...
    const existing = await db.collection('event_rsvps').findOne({ eventId, userId: user.id })
    if (!existing) return { error: 'No RSVP found', code: 'NOT_FOUND', status: 404 }
  ...
  if (path[0] === 'events' && path.length === 3 && path[2] === 'attendees' && method === 'GET') {
    ...
    const event = await db.collection('events').findOne({ id: eventId })
    if (!event || event.status === 'REMOVED') return { error: 'Event not found', code: 'NOT_FOUND', status: 404 }
  ...
  if (path[0] === 'events' && path.length === 3 && path[2] === 'report' && method === 'POST') {
    ...
    const event = await db.collection('events').findOne({ id: eventId })
    if (!event) return { error: 'Event not found', code: 'NOT_FOUND', status: 404 }
    if (user.id === event.creatorId) return { error: 'Cannot report own event', code: 'SELF_ACTION', status: 400 }
