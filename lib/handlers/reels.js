...
export async function handleReels(path, method, request, db) {
  ...
  if (path[0] === 'reels' && path.length === 3 && path[2] === 'like' && method === 'POST') {
    ...
    if (result.upsertedCount > 0) {
      ...
      await db.collection('salutes').insertOne({
        id: uuidv4(), type: 'REEL_LIKE', points: 50,
        fromUserId: user.id, toUserId: reel.creatorId, reelId,
        createdAt: new Date(),
      }).catch(() => {})
      await db.collection('users').updateOne({ id: reel.creatorId }, { $inc: { saluteCount: 50 } }).catch(() => {})
    }
  ...
  if (path[0] === 'reels' && path.length === 3 && path[2] === 'comment' && method === 'POST') {
    ...
    if (user.id !== reel.creatorId) {
      ...
      await db.collection('salutes').insertOne({
        id: uuidv4(), type: 'REEL_COMMENT', points: 200,
        fromUserId: user.id, toUserId: reel.creatorId, reelId,
        createdAt: new Date(),
      }).catch(() => {})
      await db.collection('users').updateOne({ id: reel.creatorId }, { $inc: { saluteCount: 200 } }).catch(() => {})
    }
  ...
  if (path[0] === 'reels' && path.length === 3 && path[2] === 'share' && method === 'POST') {
    ...
    if (user.id !== reel.creatorId) {
      await db.collection('salutes').insertOne({
        id: uuidv4(), type: 'REEL_SHARE', points: 100,
        fromUserId: user.id, toUserId: reel.creatorId, reelId,
        createdAt: new Date(),
      }).catch(() => {})
      await db.collection('users').updateOne({ id: reel.creatorId }, { $inc: { saluteCount: 100 } }).catch(() => {})
    }
