...
function sanitizeReel(reel, mediaAsset) {
  ...
  // URL priority: SMALLEST for fast feed load, LARGEST for HD tap
  // Instagram: feed=360p (fast), tap=720p (HD)
  const resolvedPlayback = av['360p']?.url || av['480p']?.url || av.ultrafast_720p?.url || av['720p']?.url || av.faststart?.url || asset.playbackUrl || clean.playbackUrl || ''
  ...
  // Feed playback: smallest variant for instant load
  const feedMp4 = av['360p']?.url || av['480p']?.url || av.ultrafast_720p?.url || av['720p']?.url || av.faststart?.url || resolvedPlayback
  // HD playback: largest variant for tap-to-HD
  const hdMp4 = av['720p']?.url || av['480p']?.url || av.faststart?.url || resolvedPlayback
  ...
  return {
    ...
    recommended: {
      ...
      mp4: feedMp4,        // Smallest variant — fast feed load (360p/480p)
