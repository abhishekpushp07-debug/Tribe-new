...
import { awardSalute as awardSaluteCanonical, reverseSalute as reverseSaluteCanonical } from '../services/salute-formula.js'
...
// Instagram-style count formatter: 1234 → "1.2K", 1234567 → "1.2M"
function fmtCount(n) {
  if (!n || n < 0) return '0'
...
// Duration formatter: 8000 → "0:08", 65000 → "1:05"
...
// Engagement rate: (likes + comments + saves + shares) / max(views, 1) * 100
function calcEngagementRate(reel) {
  const interactions = (reel.likeCount || 0) + (reel.commentCount || 0) + (reel.saveCount || 0) + (reel.shareCount || 0)
  const views = Math.max(reel.viewCount || 1, 1)
  return Math.round(interactions / views * 10000) / 100 // 2 decimal places
}

// Instagram-style time ago: "2h", "3d", "1w", "2mo"
function timeAgo(date) {
  ...
  if (diff < 31536000000) return Math.floor(diff / 2592000000) + 'mo'
...
function sanitizeReel(reel, mediaAsset) {
  ...
  // URL priority: 720p quality for crisp playback, original as HD fallback
  const resolvedPlayback = av['720p']?.url || av.ultrafast_720p?.url || av['480p']?.url || av.faststart?.url || asset.playbackUrl || clean.playbackUrl || ''
  ...
  // Feed: 720p quality, HD: original
  const feedMp4 = av['720p']?.url || av.ultrafast_720p?.url || av['480p']?.url || av.faststart?.url || resolvedPlayback
  // HD: original full quality for tap
  const hdMp4 = asset.publicUrl || av.faststart?.url || av['720p']?.url || resolvedPlayback
  const mediaType = bestHls ? 'hls' : bestDash ? 'dash' : 'mp4'
  const isPlayable = !!feedMp4 && feedMp4 !== '' && clean.mediaStatus !== 'UPLOADING' && clean.mediaStatus !== 'FAILED'

  return {
    id: clean.id || '', creatorId: clean.creatorId || '', caption: clean.caption || '',
    // Parsed hashtags as clickable objects [{tag, url}]
    hashtags: (clean.hashtags || []).map(h => typeof h === 'string' ? { tag: h, url: `/reels/hashtag/${h}` } : h),
    hashtagsRaw: clean.hashtags || [],
    mentions: (clean.mentions || []).map(m => typeof m === 'string' ? { username: m, url: `/users/${m}` } : m),
    mentionsRaw: clean.mentions || [],
    ...
    durationFormatted: formatDuration(clean.durationMs || (asset.duration ? asset.duration * 1000 : 0)),
    // Engagement counts
    ...
    // Engagement rate (Instagram metric)
    engagementRate: calcEngagementRate(clean),
    // Formatted counts (Instagram-style: 1.2K, 3.4M)
    formatted: {
      likes: fmtCount(clean.likeCount || 0),
      comments: fmtCount(clean.commentCount || 0),
      shares: fmtCount(clean.shareCount || 0),
      views: fmtCount(clean.viewCount || 0),
      saves: fmtCount(clean.saveCount || 0),
      watchTime: formatDuration(clean.avgWatchTimeMs || 0),
      duration: formatDuration(clean.durationMs || 0),
    },
    // Share URLs
    shareUrl: `${process.env.NEXT_PUBLIC_BASE_URL || 'https://tribeapp.pro'}/reels/${clean.id}`,
    deepLink: `tribe://reels/${clean.id}`,
    // Viewer state
    ...
    // Audio & music (Instagram-level attribution)
    audioMeta: {
      audioId: clean.audioMeta?.audioId || '',
      title: clean.audioMeta?.title || 'Original Audio',
      artist: clean.audioMeta?.artist || '',
      audioUrl: clean.audioMeta?.audioUrl || '',
      durationMs: clean.audioMeta?.durationMs || 0,
      startMs: clean.audioMeta?.startMs || 0,
      endMs: clean.audioMeta?.endMs || 0,
      isOriginal: clean.audioMeta?.isOriginal !== false,
      useCount: clean.audioMeta?.useCount || 0,
    },
    // Remix/duet
    remixOf: clean.remixOf && clean.remixOf.reelId ? {
      reelId: clean.remixOf.reelId || '',
      type: clean.remixOf.type || 'REMIX',
      creatorId: clean.remixOf.creatorId || '',
    } : {},
    isRemix: !!(clean.remixOf?.reelId),
    // Creator tools metadata
    filter: clean.filter || 'none',
    trimStartMs: clean.trimStartMs ?? 0,
    trimEndMs: clean.trimEndMs ?? 0,
    coverTimestamp: clean.coverTimestamp ?? 0,
    musicVolume: clean.musicVolume ?? 1,
    originalVolume: clean.originalVolume ?? 1,
    audioMode: clean.audioMode || 'original',
    isDraft: clean.status === 'DRAFT',
    // Flags
    syntheticDeclaration: clean.syntheticDeclaration || false,
    brandedContent: clean.brandedContent || false,
    collabCreators: clean.collabCreators || [],
    ...
    // Time ago (Instagram-style: "2h", "3d", "1w")
    timeAgo: timeAgo(clean.publishedAt || clean.createdAt),
    // Variants & feed
    ...
    // Recommended URLs
    recommended: {
      dash: bestDash, hls: bestHls,
      mp4: feedMp4,        // 720p quality — crisp playback
      hd: hdMp4,           // Largest variant — tap for HD (720p)
      poster: resolvedPoster,
      primaryUrl: bestHls || bestDash || feedMp4,
      fallbackUrl: feedMp4,
      mediaType, isPlayable,
    },
  }
}
...
export async function handleReels(path, method, request, db) {
  ...
  // ========================
  if (path[0] === 'reels' && path.length === 2 && method === 'GET' && !['feed', 'following', 'audio', 'trending', 'personalized', 'filters', 'hashtag', 'music', 'sounds', 'drafts'].includes(path[1])) {
    const viewer = await authenticate(request, db)
    const reelId = path[1]
  ...
  if (path[0] === 'reels' && path.length === 3 && path[2] === 'like' && method === 'POST') {
    ...
    if (result.upsertedCount > 0) {
      ...
      // Salute: 50 points for like
      await awardSaluteCanonical(db, { type: 'REEL_LIKE', fromUserId: user.id, toUserId: reel.creatorId, contentId: reelId, contentType: 'REEL' })
    }

    return { data: { message: 'Liked', reelId, salutes: 50 } }
  }
  ...
  if (path[0] === 'reels' && path.length === 3 && path[2] === 'comment' && method === 'POST') {
    ...
    if (user.id !== reel.creatorId) {
      ...
      await awardSaluteCanonical(db, { type: 'REEL_COMMENT', fromUserId: user.id, toUserId: reel.creatorId, contentId: reelId, contentType: 'REEL' })
    }

    return { data: { comment: sanitizeComment(comment), salutes: 200 }, status: 201 }
  }
  ...
  if (path[0] === 'reels' && path.length === 3 && path[2] === 'comments' && method === 'GET') {
    ...
    const items = comments.map(c => {
      const senderData = senderMap[c.senderId] || safeCreator(null)
      return {
        ...sanitizeComment(c),
        sender: senderData,
        author: senderData, // Frontend expects 'author'
        authorId: c.senderId || '',
      }
    })
  ...
  if (path[0] === 'reels' && path.length === 3 && path[2] === 'share' && method === 'POST') {
    ...
    if (user.id !== reel.creatorId) {
      await awardSaluteCanonical(db, { type: 'REEL_SHARE', fromUserId: user.id, toUserId: reel.creatorId, contentId: reelId, contentType: 'REEL' })
    }

    // Deep share link
    const baseUrl = process.env.NEXT_PUBLIC_BASE_URL || 'https://tribeapp.pro'
    const shareUrl = `${baseUrl}/reels/${reelId}`

    return { data: { message: 'Share tracked', reelId, shareCount, salutes: 100, shareUrl, deepLink: `tribe://reels/${reelId}` } }
  }
  ...
  if (path[0] === 'reels' && path.length === 3 && path[2] === 'view' && method === 'POST') {
    ...
    // Increment view + impression count
    await db.collection('reels').updateOne(
      { id: reelId },
      { $inc: { impressionCount: 1, viewCount: 1 }, $set: { updatedAt: new Date() } }
    )

    // Track unique viewer
    await db.collection('reel_views').updateOne(
      { reelId, userId: user.id },
      { $set: { reelId, userId: user.id, lastViewedAt: new Date() }, $inc: { count: 1 } },
      { upsert: true }
    )

    // Update unique viewer count
    const uniqueViewers = await db.collection('reel_views').countDocuments({ reelId })
    await db.collection('reels').updateOne({ id: reelId }, { $set: { uniqueViewerCount: uniqueViewers } })

    // Salute: 5pts for qualified reel view
    if (user.id !== reel.creatorId) {
      await awardSaluteCanonical(db, { type: 'REEL_VIEW', fromUserId: user.id, toUserId: reel.creatorId, contentId: reelId, contentType: 'REEL' })
    }

    return { data: { message: 'View tracked', reelId } }
  }
  ...
  // POST /admin/reels/seed-music — Seed music library (any authenticated user can trigger)
  if (path[0] === 'admin' && path[1] === 'reels' && path[2] === 'seed-music' && path.length === 3 && method === 'POST') {
    await requireAuth(request, db)
    
    const existing = await db.collection('music_library').countDocuments()
    if (existing >= 20) {
      return { data: { message: `Music already has ${existing} tracks`, count: existing } }
    }
    ...
    return { data: { message: `Seeded ${tracks.length} music tracks with real audio`, count: tracks.length } }
  }

  // POST /admin/reels/process-all — Batch-trigger HLS/DASH processing for reels with mediaId
  if (path[0] === 'admin' && path[1] === 'reels' && path[2] === 'process-all' && path.length === 3 && method === 'POST') {
    const user = await requireAuth(request, db)
    requireRole(user, 'ADMIN', 'SUPER_ADMIN')

    const url = new URL(request.url)
    const batchLimit = Math.min(parseInt(url.searchParams.get('limit') || '10'), 50)

    // Find reels with mediaId where media_assets don't have HLS yet
    const reelsWithMedia = await db.collection('reels').find(
      { mediaId: { $ne: null, $ne: '' }, status: 'PUBLISHED' },
      { projection: { _id: 0, id: 1, mediaId: 1, creatorId: 1 } }
    ).limit(batchLimit).toArray()

    const queued = []
    for (const reel of reelsWithMedia) {
      const asset = await db.collection('media_assets').findOne(
        { id: reel.mediaId, isDeleted: { $ne: true } },
        { projection: { _id: 0, id: 1, publicUrl: 1, storagePath: 1, storageType: 1, ownerId: 1, kind: 1, mimeType: 1, playbackStatus: 1, hlsUrl: 1 } }
      )
      if (!asset) continue
      if (asset.hlsUrl) continue // Already has HLS
      if (asset.playbackStatus === 'PROCESSING') continue
      if (asset.storageType !== 'SUPABASE' || !asset.publicUrl) continue
      ...
      queued.push({ reelId: reel.id, mediaId: reel.mediaId })
    }

    return { data: { message: `Queued ${queued.length} reels for HLS/DASH processing`, queued, total: reelsWithMedia.length } }
  }
  ...
  // ════════════════════════════════════════════════════════════════════
  // HASHTAG REELS — GET /reels/hashtag/:tag
  // ════════════════════════════════════════════════════════════════════
  if (path[0] === 'reels' && path[1] === 'hashtag' && path.length === 3 && method === 'GET') {
    const tag = path[2].toLowerCase().replace(/^#/, '')
    const url = new URL(request.url)
    const limit = Math.min(parseInt(url.searchParams.get('limit') || '20'), 50)
    const offset = parseInt(url.searchParams.get('offset') || '0')

    const reels = await db.collection('reels')
      .find({ hashtags: tag, status: 'PUBLISHED', visibility: 'PUBLIC' })
    ...
    const [creators, mediaAssets] = await Promise.all([
      ...
      mediaIds.length > 0 ? db.collection('media_assets').find({ id: { $in: mediaIds } }, { projection: { _id: 0, id: 1, publicUrl: 1, playbackUrl: 1, thumbnailUrl: 1, posterFrameUrl: 1, variants: 1, playbackStatus: 1, duration: 1 } }).toArray() : [],
    ])
    const cMap = Object.fromEntries(creators.map(u => [u.id, safeCreator(toUserSnippet(u))]))
    const mMap = Object.fromEntries(mediaAssets.map(a => [a.id, a]))

    const items = sanitizeReels(reels, mMap).map(r => ({ ...r, creator: cMap[r.creatorId] || safeCreator(null) }))
    const total = await db.collection('reels').countDocuments({ hashtags: tag, status: 'PUBLISHED', visibility: 'PUBLIC' })

    return { data: { hashtag: tag, reels: items, total, formatted: { total: fmtCount(total) } } }
  }

  // ════════════════════════════════════════════════════════════════════
  // REEL COVER FRAME — PATCH /reels/:id/cover
  // Creator selects timestamp for cover frame
  // ════════════════════════════════════════════════════════════════════
  if (path[0] === 'reels' && path.length === 3 && path[2] === 'cover' && method === 'PATCH') {
    const user = await requireAuth(request, db)
    const reelId = path[1]
    const reel = await db.collection('reels').findOne({ id: reelId })
    if (!reel) return { error: 'Reel not found', code: ErrorCode.NOT_FOUND, status: 404 }
    ...
    const body = await request.json()
    const { coverTimestamp, coverUrl } = body

    const updates = { updatedAt: new Date() }
    if (coverUrl) {
      updates.posterFrameUrl = coverUrl
      updates.thumbnailUrl = coverUrl
    }
    if (coverTimestamp !== undefined) {
      updates.coverTimestamp = coverTimestamp
    }

    await db.collection('reels').updateOne({ id: reelId }, { $set: updates })
    return { data: { message: 'Cover updated', reelId, coverTimestamp, coverUrl: coverUrl || reel.posterFrameUrl } }
  }

  // ════════════════════════════════════════════════════════════════════
  // REEL TRIM METADATA — PATCH /reels/:id/trim
  // Store trim start/end for playback (actual trimming done client-side or pipeline)
  // ════════════════════════════════════════════════════════════════════
  if (path[0] === 'reels' && path.length === 3 && path[2] === 'trim' && method === 'PATCH') {
    const user = await requireAuth(request, db)
    const reelId = path[1]
    const reel = await db.collection('reels').findOne({ id: reelId })
    if (!reel) return { error: 'Reel not found', code: ErrorCode.NOT_FOUND, status: 404 }
    ...
    const body = await request.json()
    const { trimStartMs, trimEndMs } = body

    if (trimStartMs !== undefined && trimEndMs !== undefined) {
      if (trimStartMs < 0 || trimEndMs <= trimStartMs) {
        return { error: 'Invalid trim range', code: ErrorCode.VALIDATION, status: 400 }
      }
      const trimDuration = trimEndMs - trimStartMs
      if (trimDuration > 90000) return { error: 'Trimmed reel must be under 90 seconds', code: ErrorCode.VALIDATION, status: 400 }

      await db.collection('reels').updateOne({ id: reelId }, {
        $set: { trimStartMs, trimEndMs, durationMs: trimDuration, updatedAt: new Date() }
      })
      return { data: { message: 'Trim applied', reelId, trimStartMs, trimEndMs, durationMs: trimDuration } }
    }
    return { error: 'trimStartMs and trimEndMs required', code: ErrorCode.VALIDATION, status: 400 }
  }

  // ════════════════════════════════════════════════════════════════════
  // REEL FILTER — PATCH /reels/:id/filter
  // Store filter name applied to reel (rendering done client-side)
  // ════════════════════════════════════════════════════════════════════
  if (path[0] === 'reels' && path.length === 3 && path[2] === 'filter' && method === 'PATCH') {
    const user = await requireAuth(request, db)
    const reelId = path[1]
    const reel = await db.collection('reels').findOne({ id: reelId })
    if (!reel) return { error: 'Reel not found', code: ErrorCode.NOT_FOUND, status: 404 }
    ...
    const body = await request.json()
    const VALID_FILTERS = ['none', 'clarendon', 'gingham', 'moon', 'lark', 'reyes', 'juno', 'slumber', 'aden', 'perpetua', 'mayfair', 'rise', 'valencia', 'xpro2', 'lo-fi', 'earlybird', 'brannan', 'inkwell', 'nashville', 'toaster']
    const filter = (body.filter || 'none').toLowerCase()

    if (!VALID_FILTERS.includes(filter)) {
      return { error: `Invalid filter. Options: ${VALID_FILTERS.join(', ')}`, code: ErrorCode.VALIDATION, status: 400 }
    }

    await db.collection('reels').updateOne({ id: reelId }, { $set: { filter, updatedAt: new Date() } })
    return { data: { message: 'Filter applied', reelId, filter, availableFilters: VALID_FILTERS } }
  }

  // ════════════════════════════════════════════════════════════════════
  // AVAILABLE FILTERS — GET /reels/filters
  // ════════════════════════════════════════════════════════════════════
  if (path[0] === 'reels' && path[1] === 'filters' && path.length === 2 && method === 'GET') {
    return { data: {
      filters: [
        { id: 'none', name: 'Normal', preview: '' },
        { id: 'clarendon', name: 'Clarendon', preview: 'brightness+contrast' },
        { id: 'gingham', name: 'Gingham', preview: 'vintage+desaturate' },
        { id: 'moon', name: 'Moon', preview: 'grayscale+high-contrast' },
        { id: 'lark', name: 'Lark', preview: 'bright+warm' },
        { id: 'reyes', name: 'Reyes', preview: 'vintage+light' },
        { id: 'juno', name: 'Juno', preview: 'warm+saturated' },
        { id: 'slumber', name: 'Slumber', preview: 'desaturated+warm' },
        { id: 'aden', name: 'Aden', preview: 'pastel+haze' },
        { id: 'perpetua', name: 'Perpetua', preview: 'aqua+warm-fade' },
        { id: 'mayfair', name: 'Mayfair', preview: 'warm+vignette' },
        { id: 'rise', name: 'Rise', preview: 'warm+glow' },
        { id: 'valencia', name: 'Valencia', preview: 'warm+faded' },
        { id: 'xpro2', name: 'X-Pro II', preview: 'vivid+vignette' },
        { id: 'lo-fi', name: 'Lo-Fi', preview: 'saturated+dark' },
        { id: 'earlybird', name: 'Earlybird', preview: 'sepia+warm' },
        { id: 'brannan', name: 'Brannan', preview: 'metallic+washed' },
        { id: 'inkwell', name: 'Inkwell', preview: 'black-and-white' },
        { id: 'nashville', name: 'Nashville', preview: 'warm+purple-tint' },
        { id: 'toaster', name: 'Toaster', preview: 'burnt+vignette' },
      ],
    }}
  }

  // ════════════════════════════════════════════════════════════════════
  // ROYALTY-FREE MUSIC — GET /reels/music/library
  // ════════════════════════════════════════════════════════════════════
  if (path[0] === 'reels' && path[1] === 'music' && path[2] === 'library' && path.length === 3 && method === 'GET') {
    const url = new URL(request.url)
    const category = url.searchParams.get('category') || 'all'
    const q = url.searchParams.get('q') || ''
    const limit = Math.min(parseInt(url.searchParams.get('limit') || '20'), 50)

    const query = { type: 'MUSIC', status: 'ACTIVE' }
    if (category !== 'all') query.category = category
    if (q) query.$or = [{ title: { $regex: q, $options: 'i' } }, { artist: { $regex: q, $options: 'i' } }]

    const tracks = await db.collection('music_library')
      .find(query, { projection: { _id: 0 } })
      .sort({ useCount: -1, createdAt: -1 })
      .limit(limit)
      .toArray()

    // If empty, return curated free tracks
    if (tracks.length === 0) {
      return { data: {
        tracks: [],
        categories: ['trending', 'chill', 'energetic', 'happy', 'sad', 'epic', 'lofi', 'pop', 'hiphop', 'electronic'],
        message: 'Music library is empty. Upload tracks via admin panel.',
      }}
    }

    return { data: { tracks, categories: ['trending', 'chill', 'energetic', 'happy', 'sad', 'epic', 'lofi', 'pop', 'hiphop', 'electronic'] } }
  }

  // ════════════════════════════════════════════════════════════════════
  // SET MUSIC ON REEL — PATCH /reels/:id/music
  // ════════════════════════════════════════════════════════════════════
  if (path[0] === 'reels' && path.length === 3 && path[2] === 'music' && method === 'PATCH') {
    const user = await requireAuth(request, db)
    const reelId = path[1]
    const reel = await db.collection('reels').findOne({ id: reelId })
    if (!reel) return { error: 'Reel not found', code: ErrorCode.NOT_FOUND, status: 404 }
    ...
    const body = await request.json()
    const { trackId, title, artist, audioUrl, durationMs, startMs, endMs } = body

    const audioMeta = {
      audioId: trackId || uuidv4(),
      title: title || 'Original Audio',
      artist: artist || user.displayName || 'Unknown',
      audioUrl: audioUrl || '',
      durationMs: durationMs || 0,
      startMs: startMs || 0,
      endMs: endMs || durationMs || 0,
      isOriginal: !trackId,
    }

    await db.collection('reels').updateOne({ id: reelId }, { $set: { audioMeta, updatedAt: new Date() } })

    // Increment use count if from library
    if (trackId) {
      await db.collection('music_library').updateOne({ id: trackId }, { $inc: { useCount: 1 } }).catch(() => {})
    }

    return { data: { message: 'Music set', reelId, audioMeta } }
  }

  // ════════════════════════════════════════════════════════════════════
  // DUET/REMIX — POST /reels/:id/duet
  // Creates a new reel linked to original via remixOf
  // ════════════════════════════════════════════════════════════════════
  if (path[0] === 'reels' && path.length === 3 && (path[2] === 'duet' || path[2] === 'remix' || path[2] === 'stitch') && method === 'POST') {
    const user = await requireAuth(request, db)
    const originalReelId = path[1]
    const remixType = path[2].toUpperCase() // DUET, REMIX, or STITCH

    const originalReel = await db.collection('reels').findOne({ id: originalReelId, status: 'PUBLISHED' })
    if (!originalReel) return { error: 'Original reel not found', code: ErrorCode.NOT_FOUND, status: 404 }

    const body = await request.json()
    const { caption, mediaUrl, mediaId, durationMs, visibility } = body
    ...
    const newReel = {
      ...
      hashtags: [],
      mentions: [],
      audioMeta: originalReel.audioMeta || {},
      durationMs: durationMs || 0,
      mediaStatus: mediaUrl ? 'READY' : (mediaId ? 'READY' : 'UPLOADING'),
      mediaId: mediaId || '',
      playbackUrl: mediaUrl || '',
      thumbnailUrl: '', posterFrameUrl: '',
      aspectRatio: '9:16', width: 0, height: 0,
      visibility: visibility || 'PUBLIC',
      moderationStatus: 'APPROVED',
      status: 'PUBLISHED',
      remixOf: { reelId: originalReelId, type: remixType, creatorId: originalReel.creatorId },
      collabCreators: [],
      seriesId: '', seriesOrder: 0,
      pinnedToProfile: false,
    ...
    if (user.id !== originalReel.creatorId) {
      await createNotification(db, originalReel.creatorId, NotificationType.REEL_COMMENT, user.id, 'REEL', newReel.id,
        `${user.displayName} created a ${remixType.toLowerCase()} of your reel`)
    }

    const { _id, ...clean } = newReel
    return { data: { reel: { ...clean, remixOf: newReel.remixOf }, originalReel: { id: originalReel.id, caption: originalReel.caption, creatorId: originalReel.creatorId } }, status: 201 }
  }

  // ════════════════════════════════════════════════════════════════════
  // DRAFT CRUD — Save/restore/delete creator drafts
  // ════════════════════════════════════════════════════════════════════

  // POST /reels/drafts — Save a new draft
  if (path[0] === 'reels' && path[1] === 'drafts' && path.length === 2 && method === 'POST') {
    const user = await requireAuth(request, db)
    const body = await request.json()
    ...
    const draft = {
      ...
      coverTimestamp: body.coverTimestamp ?? null,
      coverUrl: body.coverUrl || '',
      filter: body.filter || 'none',
      audioMeta: body.audioMeta || null,
      musicVolume: body.musicVolume ?? 1,
      originalVolume: body.originalVolume ?? 1,
      audioMode: body.audioMode || 'original', // original | music | mix | muted
      durationMs: body.durationMs || 0,
      status: 'DRAFT',
      createdAt: new Date(),
      updatedAt: new Date(),
    ...
    await db.collection('reel_drafts').insertOne(draft)
    const { _id, ...clean } = draft
    return { data: { draft: clean }, status: 201 }
  }

  // GET /reels/drafts — List my drafts (exclude filters/music static routes)
  if (path[0] === 'reels' && path[1] === 'drafts' && path.length === 2 && method === 'GET') {
    const user = await requireAuth(request, db)
    const drafts = await db.collection('reel_drafts')
      .find({ creatorId: user.id, status: 'DRAFT' }, { projection: { _id: 0 } })
      .sort({ updatedAt: -1 })
      .limit(20)
      .toArray()
    return { data: { drafts, count: drafts.length } }
  }

  // PATCH /reels/drafts/:draftId — Update a draft
  if (path[0] === 'reels' && path[1] === 'drafts' && path.length === 3 && method === 'PATCH') {
    const user = await requireAuth(request, db)
    const draftId = path[2]
    const draft = await db.collection('reel_drafts').findOne({ id: draftId, creatorId: user.id })
    if (!draft) return { error: 'Draft not found', code: ErrorCode.NOT_FOUND, status: 404 }

    const body = await request.json()
    const allowed = ['caption', 'hashtags', 'visibility', 'trimStartMs', 'trimEndMs', 'coverTimestamp', 'coverUrl', 'filter', 'audioMeta', 'musicVolume', 'originalVolume', 'audioMode', 'mediaId', 'mediaUrl', 'durationMs']
    const updates = { updatedAt: new Date() }
    for (const k of allowed) {
      if (body[k] !== undefined) updates[k] = body[k]
    }

    await db.collection('reel_drafts').updateOne({ id: draftId }, { $set: updates })
    return { data: { message: 'Draft updated', draftId } }
  }

  // DELETE /reels/drafts/:draftId — Delete a draft
  if (path[0] === 'reels' && path[1] === 'drafts' && path.length === 3 && method === 'DELETE') {
    const user = await requireAuth(request, db)
    const draftId = path[2]
    await db.collection('reel_drafts').deleteOne({ id: draftId, creatorId: user.id })
    return { data: { message: 'Draft deleted', draftId } }
  }

  // POST /reels/drafts/:draftId/publish — Publish a draft as a reel
  if (path[0] === 'reels' && path[1] === 'drafts' && path.length === 4 && path[3] === 'publish' && method === 'POST') {
    const user = await requireAuth(request, db)
    const draftId = path[2]
    const draft = await db.collection('reel_drafts').findOne({ id: draftId, creatorId: user.id, status: 'DRAFT' })
    if (!draft) return { error: 'Draft not found', code: ErrorCode.NOT_FOUND, status: 404 }
    ...
    const reel = {
      ...
      mentions: [],
      audioMeta: draft.audioMeta || null,
      durationMs: draft.trimEndMs && draft.trimStartMs ? draft.trimEndMs - draft.trimStartMs : draft.durationMs || 0,
      mediaStatus: draft.mediaId ? 'READY' : (draft.mediaUrl ? 'READY' : 'UPLOADING'),
      mediaId: draft.mediaId || '',
      playbackUrl: draft.mediaUrl || '',
      thumbnailUrl: '', posterFrameUrl: draft.coverUrl || '',
      aspectRatio: '9:16', width: 0, height: 0,
      ...
      coverTimestamp: draft.coverTimestamp,
      musicVolume: draft.musicVolume ?? 1,
      originalVolume: draft.originalVolume ?? 1,
      audioMode: draft.audioMode || 'original',
      remixOf: {}, collabCreators: [],
      seriesId: '', seriesOrder: 0,
      pinnedToProfile: false,
      likeCount: 0, commentCount: 0, saveCount: 0, shareCount: 0,
      viewCount: 0, uniqueViewerCount: 0, impressionCount: 0,
    ...
    await db.collection('reels').insertOne(reel)
    await db.collection('reel_drafts').updateOne({ id: draftId }, { $set: { status: 'PUBLISHED', publishedReelId: reel.id, updatedAt: now } })

    const { _id, ...clean } = reel
    return { data: { reel: clean, draftId, message: 'Draft published as reel' }, status: 201 }
  }
