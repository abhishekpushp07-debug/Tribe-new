...
import { getAbuseSummary } from '../services/anti-abuse-service.js'
export async function handleAdmin(path, method, request, db) {
  ...
  // ========================
  // GET /admin/abuse-dashboard — Abuse & anti-gaming admin overview
  // ========================
  if (route === 'admin/abuse-dashboard' && method === 'GET') {
    const user = await requireAuth(request, db)
    if (!['MODERATOR', 'ADMIN', 'SUPER_ADMIN'].includes(user.role)) {
      return { error: 'Admin access required', code: ErrorCode.FORBIDDEN, status: 403 }
    }
    ...
    const summary = await getAbuseSummary(db, { hours: Math.min(hours, 168) }) // cap at 7 days

    return { data: summary }
  }

  // ========================
  // GET /admin/abuse-log — Detailed abuse audit log entries
  // ========================
  if (route === 'admin/abuse-log' && method === 'GET') {
    const user = await requireAuth(request, db)
    if (!['MODERATOR', 'ADMIN', 'SUPER_ADMIN'].includes(user.role)) {
      return { error: 'Admin access required', code: ErrorCode.FORBIDDEN, status: 403 }
    }
    ...
    const url = new URL(request.url)
    const hours = parseInt(url.searchParams.get('hours') || '24', 10)
    const severity = url.searchParams.get('severity') // optional: LOW, MEDIUM, HIGH, CRITICAL
    const userId = url.searchParams.get('userId') // optional: filter by specific user
    const limit = Math.min(parseInt(url.searchParams.get('limit') || '50', 10), 200)
    const since = new Date(Date.now() - Math.min(hours, 168) * 3600000)

    const filter = { timestamp: { $gte: since } }
    if (severity) filter.severity = severity
    if (userId) filter.userId = userId

    const logs = await db.collection('abuse_audit_log')
      .find(filter, { projection: { _id: 0 } })
      .sort({ timestamp: -1 })
      .limit(limit)
      .toArray()

    return { data: { items: logs, count: logs.length, period: `${hours}h` } }
  }
