...
export async function handleMedia(path, method, request, db) {
  ...
  if (path.join('/') === 'media/upload-complete' && method === 'POST') {
    ...
    // thumbnailStatus lifecycle is managed inside generateVideoThumbnail (NONE → PENDING → READY/FAILED)
    ...
    if (asset.kind === 'VIDEO' || (asset.mimeType && asset.mimeType.startsWith('video/'))) {
      try {
        ...
        if (thumbResult) {
          ...
          // generateVideoThumbnail already set FAILED in DB if applicable
          const refreshed = await db.collection('media_assets').findOne(
            { id: mediaId }, { projection: { _id: 0, thumbnailStatus: 1 } }
          )
          thumbnailStatus = refreshed?.thumbnailStatus || 'NONE'
        }
      } catch (err) {
        const { default: logger } = await import('@/lib/logger')
