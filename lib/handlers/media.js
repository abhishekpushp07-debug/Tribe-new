...
export async function handleMedia(path, method, request, db) {
  ...
  // ========================
  if (path.join('/') === 'me/media' && method === 'GET') {
    ...
    const items = await db.collection('media_assets')
      .find(query, { projection: { _id: 0, id: 1, kind: 1, mimeType: 1, publicUrl: 1, playbackUrl: 1, thumbnailUrl: 1, posterFrameUrl: 1, sizeBytes: 1, width: 1, height: 1, duration: 1, status: 1, playbackStatus: 1, variants: 1, createdAt: 1 } })
      .sort({ createdAt: -1 })
      .limit(limit + 1)
      .toArray()
    ...
    const totalCount = await db.collection('media_assets').countDocuments({ ownerId: user.id, isDeleted: { $ne: true }, status: status || 'READY' })

    return { data: { items: results, pagination: { nextCursor, hasMore, total: totalCount }, filters: { kinds: ['IMAGE', 'VIDEO', 'AUDIO'], statuses: ['READY', 'PROCESSING', 'FAILED'] } } }
  }
  ...
  if ((path.join('/') === 'media/upload-complete' || path.join('/') === 'media/complete') && method === 'POST') {
    ...
    // Content-hash dedup: check if identical file already exists (same owner + size + mimeType)
    // If found, reuse existing asset's publicUrl to avoid double storage
    if (asset.sizeBytes) {
      const duplicate = await db.collection('media_assets').findOne({
        ...
        isDeleted: { $ne: true },
        id: { $ne: mediaId },
      }, { projection: { _id: 0, id: 1, publicUrl: 1, playbackUrl: 1, thumbnailUrl: 1, posterFrameUrl: 1, variants: 1 } })
      if (duplicate?.publicUrl && duplicate.publicUrl === asset.publicUrl) {
        // Exact same file path — skip, already uploaded
      } else if (duplicate?.publicUrl) {
        // Same size+type by same user — mark as dedup reference
        updates.dedupOf = duplicate.id
      }
    }
    ...
    // Video processing — module-aware with feature flags + duration limits
    let thumbnailStatus = 'NONE'
    let playbackStatus = 'READY'
    const isVideo = asset.kind === 'VIDEO' || (asset.mimeType && asset.mimeType.startsWith('video/'))
    const moduleType = asset.scope || 'posts'

    // Feature flags (defaults to ON — pipeline already works)
    const POST_VIDEO_PROCESSING = process.env.POST_VIDEO_PROCESSING_ENABLED !== 'false'
    const STORY_VIDEO_PROCESSING = process.env.STORY_VIDEO_PROCESSING_ENABLED !== 'false'
    const REEL_VIDEO_PROCESSING = true // Always on

    const shouldProcess = isVideo && (
      (moduleType === 'reels' && REEL_VIDEO_PROCESSING) ||
      (moduleType === 'posts' && POST_VIDEO_PROCESSING) ||
      (moduleType === 'stories' && STORY_VIDEO_PROCESSING) ||
      (!['reels', 'posts', 'stories'].includes(moduleType) && POST_VIDEO_PROCESSING)
    )

    // Duration limits: posts=5min, stories=90s, reels=90s
    const maxDurationSec = moduleType === 'stories' ? 90 : moduleType === 'reels' ? 90 : 300
    if (isVideo && duration && duration > maxDurationSec) {
      return { error: `Video too long for ${moduleType}. Max ${maxDurationSec} seconds.`, code: ErrorCode.VALIDATION, status: 400 }
    }
    ...
    if (shouldProcess) {
      ...
      await db.collection('media_assets').updateOne(
        { id: mediaId },
        { $set: { playbackStatus: 'PROCESSING', thumbnailStatus: 'PENDING', processing: { started: false }, moduleType, updatedAt: new Date() } }
      )
      ...
      await db.collection('processing_jobs').insertOne({
        ...
        outputPath: `processed/${asset.ownerId}/${mediaId}/v1/`,
        startedAt: null,
        finishedAt: null,
        durationMs: null,
        errorMessage: null,
      ...
      ;(async () => {
        ...
        try {
          ...
          const updatedAsset = { ...asset, ...updates, moduleType }
          await processVideo(db, updatedAsset)
          // Update job record
          await db.collection('processing_jobs').updateOne(
            { mediaAssetId: mediaId, status: 'queued' },
            { $set: { status: 'completed', finishedAt: new Date(), durationMs: Date.now() - jobStart } }
          ).catch(() => {})
        } catch (err) {
          const { default: logger } = await import('@/lib/logger')
          logger.error('MEDIA', 'video_pipeline_trigger_failed', { error: err.message, mediaId, moduleType })
          await db.collection('processing_jobs').updateOne(
            { mediaAssetId: mediaId },
            { $set: { status: 'failed', errorMessage: err.message, finishedAt: new Date(), durationMs: Date.now() - jobStart } }
          ).catch(() => {})
          try {
  ...
  if (path[0] === 'media' && path.length === 2 && method === 'DELETE') {
    ...
    // Cleanup processed variants from Supabase (fire-and-forget)
    if (asset.variants && typeof asset.variants === 'object') {
      ;(async () => {
        try {
          const { deleteFile } = await import('../supabase-storage.js')
          for (const [key, variant] of Object.entries(asset.variants)) {
            if (variant?.url && variant.url.includes('processed/')) {
              const path = variant.url.split('/object/public/tribe-media/')[1]
