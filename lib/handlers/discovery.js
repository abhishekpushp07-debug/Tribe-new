...
export async function handleDiscovery(path, method, request, db) {
  ...
  if (route === 'search' && method === 'GET') {
    ...
    if (type === 'all' || type === 'users') {
      ...
      // Fetch all matches (capped at 100 for ranking), then rank in-app
      const rawUsers = await db.collection('users')
        .find(userFilter)
        .limit(100)
        .toArray()
      const ranked = rawUsers.map(u => ({ ...u, _matchTier: assignUserTier(u) }))
      ranked.sort((a, b) => {
        if (a._matchTier !== b._matchTier) return a._matchTier - b._matchTier
        return (b.followersCount || 0) - (a.followersCount || 0)
      })
      totals.users = ranked.length
    ...
    // ── POSTS: recency-ranked, offset paginated ──
    if (type === 'all' || type === 'posts') {
      ...
      const [posts, pTotal] = await Promise.all([
        db.collection('content_items').find(postFilter).sort({ createdAt: -1 }).skip(offset).limit(limit).toArray(),
        db.collection('content_items').countDocuments(postFilter),
      ])
      totals.posts = pTotal
      results.posts = await enrichPosts(db, posts, user?.id || null)
    }

    // ── HASHTAGS: tiered ranking ──
    if (type === 'all' || type === 'hashtags') {
      const tagQuery = q.startsWith('#') ? q.slice(1).toLowerCase() : qLower
      const safeTagQ = tagQuery.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')
      if (safeTagQ.length >= 1) {
        const rawTags = await db.collection('hashtags')
          .find({ tag: { $regex: safeTagQ, $options: 'i' }, postCount: { $gt: 0 } })
          .limit(100)
          .toArray()
        const ranked = rawTags.map(h => ({ ...h, _matchTier: assignHashtagTier(h) }))
        ranked.sort((a, b) => {
          if (a._matchTier !== b._matchTier) return a._matchTier - b._matchTier
          return (b.postCount || 0) - (a.postCount || 0)
        })
        totals.hashtags = ranked.length
        const paged = ranked.slice(offset, offset + limit)
