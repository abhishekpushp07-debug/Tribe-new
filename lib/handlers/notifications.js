...
import { buildPushSSEStream } from '../push-notifications.js'
...
export async function handleNotifications(path, method, request, db) {
  ...
  // ========================
  // GET /notifications/stream — SSE push notification stream
  // ========================
  if (route === 'notifications/stream' && method === 'GET') {
    const user = await requireAuth(request, db)
    const sseResponse = buildPushSSEStream(request, user.id)
    return { raw: sseResponse }
  }

  // ========================
  // POST /notifications/test-push — Test push event (dev only)
  // ========================
  if (route === 'notifications/test-push' && method === 'POST') {
    const user = await requireAuth(request, db)
    const { publishPushEvent } = await import('../push-notifications.js')
    const body = await request.json()
    await publishPushEvent(user.id, body.type || 'general.announcement', body.data || { message: 'Test push notification' })
    return { data: { message: 'Test push event sent' } }
  }
