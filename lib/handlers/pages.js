...
export async function handlePages(path, method, request, db) {
  ...
  if (path[0] === 'pages' && path.length === 3 && path[2] === 'reels' && method === 'GET') {
    ...
    // Resolve media assets for poster/thumbnail/720p
    const mediaIds = [...new Set(reels.map(r => r.mediaId).filter(Boolean))]
    const mediaAssets = mediaIds.length > 0
      ? await db.collection('media_assets').find(
          { id: { $in: mediaIds } },
          { projection: { _id: 0, id: 1, publicUrl: 1, playbackUrl: 1, thumbnailUrl: 1, posterFrameUrl: 1, variants: 1, playbackStatus: 1, duration: 1 } }
        ).toArray()
      : []
    ...
    const items = reels.slice(0, limit).map(r => {
      const asset = mMap[r.mediaId] || {}
      const av = asset.variants || {}
      const thumb = r.thumbnailUrl || asset.thumbnailUrl || av.thumbnail?.url || ''
      const poster = r.posterFrameUrl || asset.posterFrameUrl || av.poster?.url || thumb
      const mp4 = av['720p']?.url || av.ultrafast_720p?.url || asset.playbackUrl || r.playbackUrl || ''
      return {
        ...r,
        playbackUrl: mp4,
        thumbnailUrl: thumb,
        posterFrameUrl: poster,
        shareUrl: `${baseUrl}/reels/${r.id}`,
        deepLink: `tribe://reels/${r.id}`,
        recommended: { mp4, poster, hd: asset.publicUrl || mp4 },
        playbackMeta: {
          loop: true, preload: 'auto', playsInline: true, muted: true,
          durationMs: r.durationMs || 0, aspectRatio: r.aspectRatio || '9:16',
        },
      }
    })
