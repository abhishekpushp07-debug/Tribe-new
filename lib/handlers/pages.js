...
export async function handlePages(path, method, request, db) {
  ...
  if (path[0] === 'pages' && path.length === 3 && path[2] === 'posts' && method === 'POST') {
    ...
    await invalidateOnEvent('POST_CREATED', { collegeId: contentItem.collegeId, houseId: null, kind: ContentKind.POST })
  ...
  if (path[0] === 'pages' && path.length === 4 && path[2] === 'posts' && method === 'PATCH') {
    ...
    const body = await request.json()
    const update = { updatedAt: new Date() }
    if (body.caption !== undefined) update.caption = String(body.caption).slice(0, Config.MAX_CAPTION_LENGTH)
