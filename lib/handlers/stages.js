...
export async function handleAppealDecision(path, method, request, db) {
  ...
  if (path[0] === 'appeals' && path.length === 3 && path[2] === 'decide' && (method === 'PATCH' || method === 'POST')) {
    ...
    if (appeal.status !== AppealStatus.PENDING && appeal.status !== 'REVIEWING') {
    ...
    if (approve && appeal.targetType === 'CONTENT') {
      await db.collection('content_items').updateOne(
        { id: appeal.targetId },
        { $set: { visibility: Visibility.PUBLIC, updatedAt: now } }
      )

      // Remove related strike
      const strike = await db.collection('strikes').findOne({ contentId: appeal.targetId })
      if (strike) {
        await db.collection('strikes').updateOne(
          { id: strike.id },
          { $set: { reversed: true, reversedBy: user.id, reversedAt: now } }
        )
    ...
    await writeAudit(db, `APPEAL_${decision}`, user.id, appeal.targetType, appeal.targetId, {
      appealId, reasonCodes, notes, approve,
    })

    return {
      data: {
        appeal: { id: appealId, status: decision, decision, decidedBy: user.id, decidedAt: now },
        message: approve ? 'Appeal approved — content restored' : 'Appeal rejected',
      },
    }
  }
...
export async function handleCollegeClaims(path, method, request, db) {
  ...
  if (path[0] === 'colleges' && path.length === 3 && path[2] === 'claim' && method === 'POST') {
    ...
    // One active claim per user per college
    const existing = await db.collection('college_claims').findOne({
      userId: user.id, collegeId, status: 'PENDING',
    })
    if (existing) return { error: 'You already have a pending claim for this college', code: ErrorCode.CONFLICT, status: 409 }

    // Cooldown: no reapply within 7 days of rejection
    ...
    if (recentRejection) {
      return { error: 'Claim was recently rejected. Please wait 7 days before reapplying', code: ErrorCode.VALIDATION, status: 400 }
    }
    ...
    await writeAudit(db, 'COLLEGE_CLAIM_SUBMITTED', user.id, 'COLLEGE', collegeId, { proofType })
  ...
  if (route === 'admin/college-claims' && method === 'GET') {
    ...
    return { data: { claims: enriched, status } }
  }
  ...
  if (path[0] === 'admin' && path[1] === 'college-claims' && path.length === 4 && path[3] === 'decide' && (method === 'PATCH' || method === 'POST')) {
    ...
    await db.collection('college_claims').updateOne(
      { id: claimId },
      { $set: { status: newStatus, reviewerId: user.id, reviewNote: note || '', reviewedAt: now } }
    )

    // If approved: link user to college
    if (approve) {
      await db.collection('users').updateOne(
        { id: claim.userId },
    ...
    await writeAudit(db, `COLLEGE_CLAIM_${newStatus}`, user.id, 'COLLEGE_CLAIM', claimId, {
      approve, note, userId: claim.userId, collegeId: claim.collegeId,
    })

    return { data: { claim: { id: claimId, status: newStatus, reviewerId: user.id, reviewedAt: now }, message: approve ? 'Claim approved' : 'Claim rejected' } }
  }
