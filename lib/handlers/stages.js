...
import { ErrorCode, Visibility, AppealStatus, ModerationAction } from '../constants.js'
...
export async function handleCollegeClaims(path, method, request, db) {
  ...
  // POST /colleges/:collegeId/claim — Submit a claim
  if (path[0] === 'colleges' && path.length === 3 && path[2] === 'claim' && method === 'POST') {
    ...
    // Block if user already verified for THIS college
    ...
    // One active (PENDING) claim per user — across ALL colleges
    const existingPending = await db.collection('college_claims').findOne({
      userId: user.id, status: 'PENDING',
    })
    if (existingPending) {
      return {
        error: `You already have a pending claim (for ${existingPending.collegeName || existingPending.collegeId}). Wait for it to be reviewed or withdraw it first.`,
        code: ErrorCode.CONFLICT,
        status: 409,
      }
    }

    // Cooldown: no reapply within 7 days of rejection for SAME college
    const recentRejection = await db.collection('college_claims').findOne({
      userId: user.id, collegeId, status: 'REJECTED',
      reviewedAt: { $gt: new Date(Date.now() - 7 * 24 * 60 * 60 * 1000) },
    })
    if (recentRejection) {
      const retryAfter = new Date(recentRejection.reviewedAt.getTime() + 7 * 24 * 60 * 60 * 1000)
      return {
        error: 'Claim was recently rejected. Please wait 7 days before reapplying.',
        code: 'COOLDOWN_ACTIVE',
        status: 429,
        data: { retryAfter: retryAfter.toISOString(), rejectedAt: recentRejection.reviewedAt },
      }
    }

    const body = await request.json()
    const { proofType, proofBlobkey } = body

    if (!proofType || !['STUDENT_ID', 'EMAIL', 'DOCUMENT', 'ENROLLMENT_NUMBER'].includes(proofType)) {
      return { error: 'proofType must be one of: STUDENT_ID, EMAIL, DOCUMENT, ENROLLMENT_NUMBER', code: ErrorCode.VALIDATION, status: 400 }
    }

    // Auto-fraud detection: 3+ rejected claims across any college → flag
    const rejectedCount = await db.collection('college_claims').countDocuments({
      userId: user.id, status: 'REJECTED',
    })
    const autoFraud = rejectedCount >= 3
    ...
    const claim = {
      ...
      proofType,
      proofBlobkey: proofBlobkey || null,
      status: 'PENDING',
      fraudSuspicion: autoFraud,
      fraudReason: autoFraud ? `Auto-flagged: ${rejectedCount} previous rejections` : null,
      reviewerId: null,
      reviewNote: null,
      reviewedAt: null,
      createdAt: new Date(),
    ...
    await writeAudit(db, 'COLLEGE_CLAIM_SUBMITTED', user.id, 'COLLEGE', collegeId, {
      proofType, fraudSuspicion: autoFraud, rejectedCount,
    })

    const { _id, ...clean } = claim
    return { data: { claim: clean }, status: 201 }
  }

  // GET /me/college-claims — User's own claims (with decision reasons visible)
  if (route === 'me/college-claims' && method === 'GET') {
    ...
    const enriched = claims.map(c => ({
      ...
      // Show retry date for rejected claims
      ...(c.status === 'REJECTED' && c.reviewedAt ? {
        retryAfter: new Date(new Date(c.reviewedAt).getTime() + 7 * 24 * 60 * 60 * 1000).toISOString(),
      } : {}),
    }))

    return { data: { claims: enriched } }
  }
  ...
  if (path[0] === 'me' && path[1] === 'college-claims' && path.length === 3 && method === 'DELETE') {
    ...
    if (claim.status !== 'PENDING') return { error: 'Only pending claims can be withdrawn', code: ErrorCode.VALIDATION, status: 400 }
    ...
    return { data: { claim: { id: claimId, status: 'WITHDRAWN' }, message: 'Claim withdrawn' } }
  }

  // GET /admin/college-claims — Admin review queue
  if (route === 'admin/college-claims' && method === 'GET') {
    const user = await requireAuth(request, db)
    if (!['MODERATOR', 'ADMIN', 'SUPER_ADMIN'].includes(user.role)) {
    ...
    const query = { status }
    if (fraudOnly) query.fraudSuspicion = true

    const claims = await db.collection('college_claims')
      .find(query, { projection: { _id: 0 } })
      .sort({ fraudSuspicion: -1, createdAt: 1 }) // fraud-flagged first, then oldest
      .limit(limit)
      .toArray()
    ...
    // Count total pending + fraud
    const totalPending = await db.collection('college_claims').countDocuments({ status: 'PENDING' })
    const totalFraudFlagged = await db.collection('college_claims').countDocuments({ status: 'PENDING', fraudSuspicion: true })

    const enriched = claims.map(c => ({ ...c, user: userMap[c.userId] || null }))

    return { data: { claims: enriched, status, totalPending, totalFraudFlagged } }
  }

  // PATCH /admin/college-claims/:claimId/flag-fraud — Admin flags claim as suspicious
  if (path[0] === 'admin' && path[1] === 'college-claims' && path.length === 4 && path[3] === 'flag-fraud' && (method === 'PATCH' || method === 'POST')) {
    ...
    if (!claim) return { error: 'Claim not found', code: ErrorCode.NOT_FOUND, status: 404 }
    if (claim.status !== 'PENDING') return { error: 'Only pending claims can be flagged', code: ErrorCode.VALIDATION, status: 400 }

    await db.collection('college_claims').updateOne(
      { id: claimId },
      { $set: { fraudSuspicion: true, fraudReason: body.reason || 'Manually flagged by admin', updatedAt: new Date() } }
    )

    await writeAudit(db, 'COLLEGE_CLAIM_FRAUD_FLAGGED', user.id, 'COLLEGE_CLAIM', claimId, {
      reason: body.reason, userId: claim.userId, collegeId: claim.collegeId,
    })

    return { data: { claim: { id: claimId, fraudSuspicion: true, fraudReason: body.reason || 'Manually flagged' } } }
  }

  // PATCH /admin/college-claims/:claimId/decide — Admin decision
  if (path[0] === 'admin' && path[1] === 'college-claims' && path.length === 4 && path[3] === 'decide' && (method === 'PATCH' || method === 'POST')) {
    ...
    const claim = await db.collection('college_claims').findOne({ id: claimId })
    if (!claim) return { error: 'Claim not found', code: ErrorCode.NOT_FOUND, status: 404 }
    if (claim.status !== 'PENDING') return { error: 'Claim already decided', code: ErrorCode.CONFLICT, status: 409 }
    ...
    await db.collection('college_claims').updateOne(
      { id: claimId },
      { $set: { status: newStatus, reviewerId: user.id, reviewNote: note || '', reviewedAt: now, updatedAt: now } }
    )

    // If approved: link user to college + set verified
    if (approve) {
      // If user was previously linked to a different college, decrement old college
      const claimUser = await db.collection('users').findOne({ id: claim.userId })
      if (claimUser?.collegeId && claimUser.collegeId !== claim.collegeId) {
        await db.collection('colleges').updateOne(
          { id: claimUser.collegeId },
          { $inc: { membersCount: -1 } }
        )
      }
      ...
      await db.collection('users').updateOne(
        { id: claim.userId },
        { $set: { collegeId: claim.collegeId, collegeVerified: true, updatedAt: now } }
      )
      await db.collection('colleges').updateOne(
        { id: claim.collegeId },
        { $inc: { membersCount: 1 } }
    ...
    await writeAudit(db, `COLLEGE_CLAIM_${newStatus}`, user.id, 'COLLEGE_CLAIM', claimId, {
      approve, note, userId: claim.userId, collegeId: claim.collegeId, fraudSuspicion: claim.fraudSuspicion,
    })
    ...
    await db.collection('notifications').insertOne({
      ...
      message: approve
        ...
        : `Your college claim for ${claim.collegeName} has been rejected.${note ? ' Reason: ' + note : ''}`,
      metadata: { claimId, collegeId: claim.collegeId, collegeName: claim.collegeName, decision: newStatus, note: note || '' },
      read: false,
      createdAt: now,
    })
    ...
    return {
      data: {
        claim: { id: claimId, status: newStatus, reviewerId: user.id, reviewedAt: now, collegeName: claim.collegeName },
        message: approve ? 'Claim approved — user verified' : 'Claim rejected',
      },
    }
  }
