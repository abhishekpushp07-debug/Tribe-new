...
export async function handleTribeAdmin(path, method, request, db) {
  ...
  if (path[0] === 'admin' && path[1] === 'tribes' && path[2] === 'reassign' && path.length === 3 && method === 'POST') {
    ...
    if (!userId || !tribeCode || !reason) {
      return { error: 'userId, tribeCode, and reason are required', code: 'VALIDATION_ERROR', status: 400 }
    }

    const targetUser = await db.collection('users').findOne({ id: userId })
    if (!targetUser) return { error: 'User not found', code: 'NOT_FOUND', status: 404 }

    const newTribe = await db.collection('tribes').findOne({ tribeCode: tribeCode.toUpperCase() })
    if (!newTribe) return { error: 'Tribe not found', code: 'NOT_FOUND', status: 404 }
  ...
  if (path[0] === 'admin' && path[1] === 'tribe-seasons' && path.length === 2 && method === 'POST') {
    ...
    // Create new season
    if (!name || !year) return { error: 'name and year required', code: 'VALIDATION_ERROR', status: 400 }
  ...
  if (path[0] === 'admin' && path[1] === 'tribe-contests' && path.length === 2 && method === 'POST') {
    ...
    if (!seasonId || !name) return { error: 'seasonId and name required', code: 'VALIDATION_ERROR', status: 400 }
  ...
  if (path[0] === 'admin' && path[1] === 'tribe-contests' && path.length === 4 && path[3] === 'resolve' && method === 'POST') {
    ...
    if (!winnerTribeId) return { error: 'winnerTribeId required', code: 'VALIDATION_ERROR', status: 400 }
    ...
    if (contest.status === 'RESOLVED') return { error: 'Contest already resolved', code: 'CONFLICT', status: 409 }
  ...
  if (path[0] === 'admin' && path[1] === 'tribe-awards' && path[2] === 'resolve' && path.length === 3 && method === 'POST') {
    ...
    if (!seasonId) return { error: 'seasonId required', code: 'VALIDATION_ERROR', status: 400 }
    ...
    if (standings.length === 0) return { error: 'No standings for this season', code: 'VALIDATION_ERROR', status: 400 }
    ...
    if (existingAward) return { error: 'Award already resolved for this season', code: 'CONFLICT', status: 409 }
  ...
  if (path[0] === 'admin' && path[1] === 'tribes' && path[2] === 'boards' && path.length === 3 && method === 'POST') {
    ...
    if (!tribeId || !members || !Array.isArray(members)) {
      return { error: 'tribeId and members[] required', code: 'VALIDATION_ERROR', status: 400 }
    }
