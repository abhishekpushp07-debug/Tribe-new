...
export async function handleTribes(path, method, request, db) {
  ...
  if (path[0] === 'tribes' && path[1] === 'leaderboard' && path.length === 2 && method === 'GET') {
    ...
    // Aggregate engagement metrics per tribe using $facet for efficiency
    const postDateFilter = since ? { createdAt: { $gte: since } } : {}

    const leaderboard = await Promise.all(tribes.map(async (tribe) => {
      const tribeId = tribe.id

      // Get all member userIds for this tribe
      const memberIds = await db.collection('users')
        .find({ tribeId }, { projection: { _id: 0, id: 1 } })
        .toArray()
      ...
      if (memberIds.length === 0) {
        return {
          ...
          quote: tribe.quote,
          membersCount: 0,
          metrics: { uploads: 0, posts: 0, reels: 0, stories: 0, likesReceived: 0, commentsReceived: 0, sharesReceived: 0, viralReels: 0 },
          engagementScore: 0,
          scoreBreakdown: { uploadPoints: 0, likePoints: 0, commentPoints: 0, sharePoints: 0, viralBonus: 0 },
        }
      }

      // Run all aggregations in parallel
      const [
        postAgg, reelAgg, storyCount,
        postEngAgg, reelEngAgg, viralReelCount,
      ] = await Promise.all([
        // Post count + engagement (likes, comments, shares)
        db.collection('content_items').aggregate([
          { $match: { authorId: { $in: memberIds }, isDeleted: { $ne: true }, ...postDateFilter } },
          { $group: { _id: null, count: { $sum: 1 }, likes: { $sum: '$likeCount' }, comments: { $sum: '$commentCount' }, shares: { $sum: '$shareCount' } } },
        ]).toArray(),
        // Reel count + engagement
        db.collection('reels').aggregate([
          { $match: { creatorId: { $in: memberIds }, isDeleted: { $ne: true }, ...postDateFilter } },
          { $group: { _id: null, count: { $sum: 1 }, likes: { $sum: '$likeCount' }, comments: { $sum: '$commentCount' }, shares: { $sum: '$shareCount' } } },
        ]).toArray(),
        // Story count
        ...
        // Post-level engagement totals (already in postAgg, but separate for clarity)
        db.collection('content_items').aggregate([
          { $match: { authorId: { $in: memberIds }, isDeleted: { $ne: true }, ...postDateFilter } },
          { $group: { _id: null, likes: { $sum: '$likeCount' }, comments: { $sum: '$commentCount' }, shares: { $sum: '$shareCount' } } },
        ]).toArray(),
        // Reel-level engagement totals
        db.collection('reels').aggregate([
          { $match: { creatorId: { $in: memberIds }, isDeleted: { $ne: true }, ...postDateFilter } },
          { $group: { _id: null, likes: { $sum: '$likeCount' }, comments: { $sum: '$commentCount' }, shares: { $sum: '$shareCount' } } },
        ]).toArray(),
        // Viral reels (1000+ likes) — bonus
      ...
      const postCount = postAgg[0]?.count || 0
      const reelCount = reelAgg[0]?.count || 0

      // Total uploads (posts + reels + stories)
      const totalUploads = postCount + reelCount + storyCount

      // Combine engagement from posts + reels
      const totalLikes = (postEngAgg[0]?.likes || 0) + (reelEngAgg[0]?.likes || 0)
      const totalComments = (postEngAgg[0]?.comments || 0) + (reelEngAgg[0]?.comments || 0)
      const totalShares = (postEngAgg[0]?.shares || 0) + (reelEngAgg[0]?.shares || 0)

      // Scoring formula:
      // Upload (post/reel/story): 100 pts each
      // Like received: 10 pts each
      // Comment received: 20 pts each
      // Share received: 50 pts each
      // Viral reel bonus (1000+ likes): 1000 pts each
      const uploadPoints = totalUploads * 100
      const likePoints = totalLikes * 10
      const commentPoints = totalComments * 20
      const sharePoints = totalShares * 50
      const viralBonus = viralReelCount * 1000
      const engagementScore = uploadPoints + likePoints + commentPoints + sharePoints + viralBonus
    ...
    // Sort by engagement score descending, assign ranks
