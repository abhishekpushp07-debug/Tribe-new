...
export async function handleTribes(path, method, request, db) {
  ...
  // ========================
  // GET /tribe-rivalries — List active and recent rivalries
  // ========================
  if (path[0] === 'tribe-rivalries' && path.length === 1 && method === 'GET') {
    ...
    const [rivalries, total] = await Promise.all([
      db.collection('tribe_rivalries').find(filter).sort({ createdAt: -1 }).skip(offset).limit(limit).toArray(),
    ...
    return { data: { items: enriched, total, limit, offset } }
  }
