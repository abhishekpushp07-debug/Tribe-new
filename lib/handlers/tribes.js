    ...
    const leaderboard = await Promise.all(tribes.map(async (tribe) => {
      ...
      if (memberIds.length === 0) {
        return {
          ...
          quote: tribe.quote,
          membersCount: 0,
          metrics: { posts: 0, reels: 0, likesReceived: 0, followersTotal: 0, activeMemberCount: 0 },
          engagementScore: 0,
        }
      }

      // Run aggregations in parallel
      const [postCount, reelCount, likeAgg, followerAgg, activeMemberAgg] = await Promise.all([
        // Posts created by tribe members
        db.collection('content_items').countDocuments({
          authorId: { $in: memberIds },
          isDeleted: { $ne: true },
          ...postDateFilter,
        }),
        // Reels created by tribe members
        db.collection('reels').countDocuments({
          creatorId: { $in: memberIds },
          isDeleted: { $ne: true },
          ...postDateFilter,
        }),
        // Total likes received on tribe members' content
        db.collection('content_items').aggregate([
          { $match: { authorId: { $in: memberIds }, isDeleted: { $ne: true }, ...postDateFilter } },
          { $group: { _id: null, total: { $sum: '$likeCount' } } },
        ]).toArray(),
        // Total followers of tribe members
        db.collection('users').aggregate([
          { $match: { id: { $in: memberIds } } },
          { $group: { _id: null, total: { $sum: '$followersCount' } } },
        ]).toArray(),
        // Active members (posted in the period)
        db.collection('content_items').distinct('authorId', {
          authorId: { $in: memberIds },
          isDeleted: { $ne: true },
          ...postDateFilter,
        }),
      ...
      const followersTotal = followerAgg[0]?.total || 0
      const activeMemberCount = activeMemberAgg.length

      // Engagement score: weighted composite
      // Posts: 5pts, Reels: 10pts, Likes: 2pts, Followers: 1pt, Active members: 20pts
      const engagementScore = (postCount * 5) + (reelCount * 10) + (likesReceived * 2) + (followersTotal * 1) + (activeMemberCount * 20)
