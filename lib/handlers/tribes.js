...
export async function handleTribes(path, method, request, db) {
  ...
  if (path[0] === 'tribes' && path[1] === 'me' && path[2] === 'welcome' && path.length === 3 && method === 'GET') {
    ...
    return { data: { user: { id: user.id, displayName: user.displayName, tribeCode: user.tribeCode || '' }, tribe: { code: tc.tribeCode, name: tc.tribeName, heroName: tc.heroName, pvcName: tc.paramVirChakraName || tc.heroName, rank: tc.rank || '', unit: tc.unit || '', war: tc.war || '', dateOfAction: tc.dateOfAction || '', status: tc.status || '', biography: tc.biography || '', quote: tc.quote || '', animalIcon: tc.animalIcon || '', primaryColor: tc.primaryColor || '', secondaryColor: tc.secondaryColor || '' }, welcome: { headline: 'Welcome to ' + tc.tribeName + '!', message: 'You are part of ' + tc.tribeName + ', named after ' + tc.heroName + '. ' + (tc.quote || '') + ' Earn salutes for your tribe!', cta: 'Start Contributing' } } }
  }
  ...
  // GET /tribes/:id/board/current — check snapshot first, fallback to live
  if (path[0] === 'tribes' && path.length === 4 && path[2] === 'board' && path[3] === 'current' && method === 'GET') {
    ...
    const ml = now.toLocaleString('en', { month: 'long' }) + ' ' + now.getFullYear()
    const tc = TRIBES.find(t => t.tribeCode === tribe.code) || {}

    // Check frozen snapshot first
    const snapshot = await db.collection('board_snapshots').findOne({ tribeId: tribe.id, monthKey })
    if (snapshot) {
      return { data: { tribe: { id: tribe.id, code: tribe.code, name: tribe.tribeName || tribe.name, heroName: tc.heroName || '', primaryColor: tc.primaryColor || '' }, month: ml, monthKey, board: snapshot.board, boardSize: 7, frozen: true, frozenAt: snapshot.frozenAt } }
    }

    // Live query
    const ms = new Date(now.getFullYear(), now.getMonth(), 1)
    const board = await db.collection('salutes').aggregate([{ $match: { createdAt: { $gte: ms }, reversed: { $ne: true } } }, { $lookup: { from: 'users', localField: 'toUserId', foreignField: 'id', as: 'u' } }, { $unwind: '$u' }, { $match: { 'u.tribeId': tribe.id } }, { $group: { _id: '$toUserId', t: { $sum: '$points' }, n: { $first: '$u.displayName' }, a: { $first: '$u.profilePicUrl' }, un: { $first: '$u.username' } } }, { $sort: { t: -1, _id: 1 } }, { $limit: 7 }]).toArray()
    return { data: { tribe: { id: tribe.id, code: tribe.code, name: tribe.tribeName || tribe.name, heroName: tc.heroName || '', primaryColor: tc.primaryColor || '' }, month: ml, monthKey, board: board.map((b, i) => ({ rank: i + 1, userId: b._id, displayName: b.n || '', username: b.un || '', avatarUrl: b.a || '', salutes: b.t })), boardSize: 7, frozen: false } }
  }

  // GET /tribes/:id/board/:monthKey — Historical board (YYYY-MM)
  if (path[0] === 'tribes' && path.length === 4 && path[2] === 'board' && /^\d{4}-\d{2}$/.test(path[3]) && method === 'GET') {
    const tribe = await db.collection('tribes').findOne({ $or: [{ id: path[1] }, { code: path[1] }] }); if (!tribe) return { error: 'Tribe not found', code: ErrorCode.NOT_FOUND, status: 404 }
    const snapshot = await db.collection('board_snapshots').findOne({ tribeId: tribe.id, monthKey: path[3] })
    if (!snapshot) return { data: { tribe: { id: tribe.id, code: tribe.code }, month: path[3], board: [], frozen: false, message: 'No board snapshot for this month' } }
    const tc = TRIBES.find(t => t.tribeCode === tribe.code) || {}
    return { data: { tribe: { id: tribe.id, code: tribe.code, name: tribe.tribeName || tribe.name, heroName: tc.heroName || '', primaryColor: tc.primaryColor || '' }, month: path[3], board: snapshot.board, boardSize: 7, frozen: true, frozenAt: snapshot.frozenAt } }
  }

  // POST /admin/tribes/freeze-boards — Freeze current month boards for ALL tribes (idempotent)
  if (path[0] === 'admin' && path[1] === 'tribes' && path[2] === 'freeze-boards' && path.length === 3 && method === 'POST') {
    const user = await requireAuth(request, db)
    const now = new Date(); const monthKey = `${now.getFullYear()}-${String(now.getMonth()+1).padStart(2,'0')}`
    const ms = new Date(now.getFullYear(), now.getMonth(), 1)
    const allTribes = await db.collection('tribes').find({}, { projection: { _id: 0, id: 1, code: 1, tribeName: 1 } }).toArray()
    let frozen = 0; let skipped = 0
    for (const tribe of allTribes) {
      const existing = await db.collection('board_snapshots').findOne({ tribeId: tribe.id, monthKey })
      if (existing) { skipped++; continue }
      const board = await db.collection('salutes').aggregate([{ $match: { createdAt: { $gte: ms }, reversed: { $ne: true } } }, { $lookup: { from: 'users', localField: 'toUserId', foreignField: 'id', as: 'u' } }, { $unwind: '$u' }, { $match: { 'u.tribeId': tribe.id } }, { $group: { _id: '$toUserId', t: { $sum: '$points' }, n: { $first: '$u.displayName' }, a: { $first: '$u.profilePicUrl' }, un: { $first: '$u.username' } } }, { $sort: { t: -1, _id: 1 } }, { $limit: 7 }]).toArray()
      await db.collection('board_snapshots').insertOne({ tribeId: tribe.id, monthKey, board: board.map((b, i) => ({ rank: i + 1, userId: b._id, displayName: b.n || '', username: b.un || '', avatarUrl: b.a || '', salutes: b.t })), frozenAt: new Date(), frozenBy: user.id })
      frozen++
    }
    return { data: { message: `Board freeze complete`, monthKey, frozen, skipped, total: allTribes.length } }
  }
