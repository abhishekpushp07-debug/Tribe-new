...
export async function handleFeed(path, method, request, db) {
  ...
  if (route === 'feed/stories' && method === 'GET') {
    ...
    // Enrich authors + resolve avatar CDN URLs
    const authorIds = Object.keys(grouped)
    const authors = await db.collection('users').find({ id: { $in: authorIds } }, { projection: { _id: 0, id: 1, displayName: 1, username: 1, avatarMediaId: 1, avatar: 1, profilePicUrl: 1, role: 1, tribeId: 1, tribeCode: 1, tribeName: 1, tribeHeroName: 1, collegeName: 1, collegeId: 1 } }).toArray()
    
    // Batch-resolve avatar CDN URLs
    ...
    const authorMap = Object.fromEntries(authors.map(a => {
      const snippet = sanitizeUser(a)
      if (snippet) {
        const mid = a.avatarMediaId || a.avatar
        const cdnUrl = mid ? avatarCdnMap[mid] : null
        snippet.avatarUrl = cdnUrl || a.profilePicUrl || snippet.avatarUrl || ''
        snippet.profilePicUrl = a.profilePicUrl || cdnUrl || ''
      }
      return [a.id, snippet]
    }))
    ...
    const storyRail = authorIds.map(authorId => ({
      ...
      stories: grouped[authorId].map(s => {
        ...
        if (clean.createdAt) {
          ...
          if (diff < 60000) ta = 'now'
          else if (diff < 3600000) ta = Math.floor(diff / 60000) + 'm'
          else if (diff < 86400000) ta = Math.floor(diff / 3600000) + 'h'
          else ta = Math.floor(diff / 86400000) + 'd'
        }
        return { ...clean, viewCount: clean.viewCount || 0, reactionCount: clean.reactionCount || 0, replyCount: clean.replyCount || 0, timeAgo: ta, formatted: { views: String(clean.viewCount || 0), reactions: String(clean.reactionCount || 0) } }
      }),
