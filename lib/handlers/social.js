...
async function awardSalute(db, type, points, fromUserId, toUserId, contentId) {
  if (fromUserId === toUserId) return // No self-salute
  await db.collection('salutes').insertOne({
    id: uuidv4(), type, points, fromUserId, toUserId, contentId, createdAt: new Date(),
  }).catch(() => {})
  await db.collection('users').updateOne({ id: toUserId }, { $inc: { saluteCount: points } }).catch(() => {})
}
...
export async function handleSocial(path, method, request, db) {
  ...
  if (path[0] === 'content' && path.length === 3 && path[2] === 'like' && method === 'POST') {
    ...
    if (post.authorId !== user.id) {
      ...
      await awardSalute(db, 'POST_LIKE', 50, user.id, targetId, contentId)
    }
  ...
  if (path[0] === 'content' && path.length === 3 && path[2] === 'comments' && method === 'POST') {
    ...
    if (user.id !== post.authorId) {
      const targetId = post.authorType === 'PAGE' ? post.actingUserId || post.authorId : post.authorId
      await awardSalute(db, 'POST_COMMENT', 200, user.id, targetId, contentId)
    }
  ...
  if (path[0] === 'content' && path.length === 3 && path[2] === 'share' && method === 'POST') {
    ...
    if (user.id !== original.authorId) {
      ...
      await awardSalute(db, 'POST_SHARE', 100, user.id, targetId, originalContentId)
    }
