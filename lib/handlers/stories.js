...
function sanitizeStory(story) {
  ...
  const hasVideo = clean.type === 'VIDEO' || (Array.isArray(clean.media) && clean.media.some(m => (m.mimeType || m.type || '').toLowerCase().includes('video')))
  
  // Time ago
  let timeAgo = ''
  if (clean.createdAt) {
    const diff = Date.now() - new Date(clean.createdAt).getTime()
    if (diff < 60000) timeAgo = 'now'
    else if (diff < 3600000) timeAgo = Math.floor(diff / 60000) + 'm'
    else if (diff < 86400000) timeAgo = Math.floor(diff / 3600000) + 'h'
    else timeAgo = Math.floor(diff / 86400000) + 'd'
  }
  ...
  return {
    ...
    replyCount: clean.replyCount || 0,
    timeAgo,
    formatted: {
      views: String(clean.viewCount || 0),
      reactions: String(clean.reactionCount || 0),
    },
    media: Array.isArray(clean.media) ? clean.media.map(m => ({
      ...m,
    ...
    ...(hasVideo ? {
      playbackMeta: {
        ...
        muted: true,
        autoAdvance: true,
        durationHint: clean.media?.[0]?.duration || null,
      },
    } : {}),
  }
}
...
export async function handleStories(path, method, request, db) {
  ...
  if (route === 'stories' && method === 'POST') {
    ...
    if (mediaIds?.length > 0) {
      ...
      media = assets.map(a => ({
        ...
        posterFrameUrl: a.thumbnailUrl || null,
      }))
    }
    ...
    // Content moderation — ASYNC (non-blocking for instant story publish)
    // Story is created FIRST, moderation runs in background
    ...
    const story = {
      ...
      status: 'ACTIVE', // Publish instantly — moderate in background
      viewCount: 0,
      reactionCount: 0,
      replyCount: 0,
      moderation: null,
    ...
    // Fire-and-forget: background moderation (doesn't block response)
    if (textToModerate.length > 0) {
      ;(async () => {
        try {
          const modResult = await moderateCreateContent(db, {
            entityType: 'story', actorUserId: user.id,
            collegeId: user.collegeId, tribeId: user.tribeId,
            caption: textToModerate, text: textToModerate,
            metadata: { route: 'POST /stories', type: storyType },
          })
          const decision = modResult.decision
          if (decision.action === 'ESCALATE') {
            await db.collection('stories').updateOne({ id: story.id }, {
              $set: { status: 'HELD', moderation: { action: decision.action, provider: decision.provider, confidence: decision.confidence, checkedAt: new Date() }, updatedAt: new Date() }
            })
          } else {
            await db.collection('stories').updateOne({ id: story.id }, {
              $set: { moderation: { action: decision.action, provider: decision.provider, confidence: decision.confidence, checkedAt: new Date() } }
            })
          }
        } catch {}
      })()
  ...
  if (path[0] === 'stories' && path.length === 3 && path[2] === 'react' && method === 'POST') {
    ...
    const storyId = path[1]
    const body = await request.json()
    const { emoji: rawEmoji } = body

    // Unicode NFC normalization — fixes encoding mismatch (variation selectors, etc.)
    const emoji = rawEmoji ? rawEmoji.normalize('NFC') : ''
    const validReactions = StoryConfig.VALID_REACTIONS.map(e => e.normalize('NFC'))
