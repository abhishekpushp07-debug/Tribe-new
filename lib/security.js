...
async function initRateLimitRedis() {
  ...
  try {
    ...
    rlRedis.on('close', () => { rlRedisReady = false })
    rlRedis.on('ready', () => { rlRedisReady = true })
...
// ========== CANONICAL AUDIT LOGGER (Stage 3: Unified pipeline) ==========
...
// Schema: { id, category, eventType, actorId, targetType, targetId, ip, userAgent,
//           requestId, metadata (PII-masked), severity, createdAt }
