  ...
  return posts.map(p => {
    ...
    if (Array.isArray(clean.media) && clean.media.length > 0) {
      resolvedMedia = clean.media.map(m => {
        ...
        // Use SMALLEST variant for fast feed load, provide HD option
        const variants = asset.variants || {}
        const feedUrl = (isVideo && (variants['360p']?.url || variants['480p']?.url || variants.ultrafast_720p?.url || variants['720p']?.url || variants.faststart?.url))
          || asset.playbackUrl || asset.publicUrl || m.publicUrl || m.url
        const hdUrl = (isVideo && (variants['720p']?.url || variants['480p']?.url || variants.faststart?.url))
          || asset.playbackUrl || asset.publicUrl || m.publicUrl || m.url
