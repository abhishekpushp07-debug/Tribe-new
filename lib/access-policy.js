 ...
 *   B3: For PAGE-authored content, ownership check uses actingUserId
 */
export function canViewContent(content, viewerId, viewerRole) {
  if (!content) return false
  const v = content.visibility
  // B3: For page-authored content, the "owner" is the acting user
  const ownerId = (content.authorType === 'PAGE' && content.actingUserId) ? content.actingUserId : content.authorId
 ...
 * B3: For page-authored content, block check uses actingUserId (the real user who posted).
 *
...
export async function applyFeedPolicy(db, viewerId, viewerRole, items) {
  ...
  if (viewerId) {
    // B3: For PAGE-authored content, use actingUserId for block check
    const authorIds = [...new Set(filtered.map(i => {
      if (i.authorType === 'PAGE' && i.actingUserId) return i.actingUserId
      return i.authorId
    }).filter(Boolean))]
    if (authorIds.length) {
      ...
      if (blockedIds.size > 0) {
        filtered = filtered.filter(i => {
          const checkId = (i.authorType === 'PAGE' && i.actingUserId) ? i.actingUserId : i.authorId
          return !blockedIds.has(checkId)
        })
      }
    }
  }
